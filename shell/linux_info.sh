...
Numbers=$(grep -c 'lm' /proc/cpuinfo)
...
#Memtotal 内存总大小
Total=$(< /proc/meminfo grep 'MemTotal' |awk -F : '{print $2}' |sed 's/^[ \t]*//g')
echo -e "Total Memory:\n${Total}\n${Line}"
...
#下面为统计内存条数
Number=$(dmidecode | grep -A16 "Memory Device$" |grep Size|sort |sed 's/^[ \t]*//g'| grep -v -c 'No Module Installed')
echo -e "Number of Physical Memory:\n${Number}\n${Line}"


#SwapTotal swap分区总大小
SwapTotal=$(< /proc/meminfo grep 'SwapTotal' |awk -F : '{print $2}' |sed 's/^[ \t]*//g')
echo -e "Total Swap:\n${SwapTotal}\n${Line}"

#Buffers size
Buffers=$(< /proc/meminfo grep 'Buffers' |awk -F : '{print $2}' |sed 's/^[ \t]*//g')
echo -e "Buffers:\n${Buffers}\n${Line}"

#Cached size
Cached=$(< /proc/meminfo grep '\<Cached\>' |awk -F : '{print $2}' |sed 's/^[ \t]*//g')
echo -e "Cached:\n${Cached}\n${Line}"
